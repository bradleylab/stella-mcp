...
| --- | --- | --- |
| Model core | `model_types`, `model`, `model_layout`, `xmile_parse`, `xmile_export`, `validator`, `render_svg` | Uses the Python standard library and project modules. It must not require simulation dependencies. |
...
| `model_layout.py` | Dependency graphs, subsystem placement, stock sizing, flow routing, connector angles, collision detection, and collision resolution. |
...
`StellaModel` keeps its established layout and XML methods as delegates to
`model_layout` and `xmile_export`. Callers should continue importing public
...
Version 0.11 retains these public surfaces from 0.10:
...
- generated XMILE, model snapshots, layout behavior, and SVG rendering;
